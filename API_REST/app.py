...
# Vérifie si une valeur est déjà un mot de passe crypté en bcrypt
def is_bcrypt_hash(value):
    return isinstance(value, str) and re.match(r'^\$2[aby]\$.{56}$', value) is not None


# Convertit une date reçue en format compatible avec MySQL
...
# Récupère les lignes d'une table avec les colonnes demandées
...
# Endpoint qui récupère les scans et les retourne au format JSON
...
# Endpoint qui récupère les clés USB détectées et les retourne au format JSON
...
# Endpoint qui récupère les utilisateurs et les retourne au format JSON
...
# Endpoint qui crée un nouvel utilisateur dans la base de données
...
# Endpoint qui modifie les informations d'un utilisateur existant
...
# Endpoint qui importe une liste d'utilisateurs depuis un fichier CSV
...
# Endpoint qui supprime un utilisateur avec son identifiant
...
# Endpoint qui récupère les logs d'accès et les retourne au format JSON
...
# Endpoint qui récupère les actions des administrateurs et les retourne au format JSON
...
# Endpoint qui importe les logs USB, fichiers ou scans dans la base de données
...
def import_logs():
    ...
    if log_type == 'usb':
        ...
        return jsonify({"error": "Type de log non supporté"}), 400
    ...
    try:
        ...
        conn.commit()
        return jsonify({"message": f"Logs {log_type} importés avec succès"}), 200
    except Exception as e:
        conn.rollback()
        return jsonify({"error": str(e)}), 500
    finally:
...
# Endpoint qui liste les rapports disponibles
...
# Endpoint qui récupère le contenu d'un rapport spécifique
